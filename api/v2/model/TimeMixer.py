...
class TimeMixer(nn.Module):
    ...
    def _prepare_inputs(self, x_enc, x_mark_enc=None, normalize=True):
        x_list, x_mark_list = [], []
        for i, x in enumerate(x_enc):
            ...
            if x_mark_enc is not None:
                x_mark_list.append(x_mark_enc[i].repeat(N, 1, 1))
        return x_list, x_mark_list if x_mark_enc is not None else None, B, N

    def _encode(self, x_list, x_mark_list=None):
        enc_out_list = []
        for i, x in enumerate(x_list):
            x_mark = x_mark_list[i] if x_mark_list else None
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        x_list, x_mark_list, B, _ = self._prepare_inputs(x_enc, x_mark_enc)
        enc_out_list = self._encode(x_list, x_mark_list)

        # Future Multipredictor Mixing
        dec_out_list = self._future_multi_mixing(B, enc_out_list)

        dec_out = torch.stack(dec_out_list, dim=-1).sum(-1)
        dec_out = self.normalize_layers[0](dec_out, 'denorm')
        return dec_out

    def _future_multi_mixing(self, B, enc_out_list):
        dec_out_list = []
    ...
    def classification(self, x_enc, x_mark_enc):
        ...
        enc_out_list = self._encode(x_enc)
    ...
    def anomaly_detection(self, x_enc):
        ...
        x_list, _, B, N = self._prepare_inputs(x_enc)
        enc_out_list = self._encode(x_list)
    ...
    def imputation(self, x_enc, x_mark_enc, mask):
        ...
        B, _, _ = x_enc.size()
        x_enc, _ = self.__multi_scale_process_inputs(x_enc, x_mark_enc)

        x_list, _, B, _ = self._prepare_inputs(x_enc, normalize=False)
        enc_out_list = self._encode(x_list)
